  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div>
          ...
          {row.description && <span style={{ fontSize: '0.75rem', color: '#64748b' }}>{row.description}</span>}
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <span style={{ fontSize: '0.8125rem', fontWeight: '600', color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}>
          {row.offer_scope === 'store' && <><Store size={14} color="#2563eb" /> Entire Store</>}
          {row.offer_scope === 'category' && <><Layers size={14} color="#f59e0b" /> Category: {row.category_name || 'All Categories'}</>}
          {row.offer_scope === 'product' && <><Package size={14} color="#8b5cf6" /> Product: {row.product_name || 'Selected Item'}</>}
        </span>
      )
    },
    {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: '800', color: '#0f172a', margin: 0 }}>Offer & Discount Management</h1>
          <p style={{ color: '#64748b', fontSize: '0.875rem', margin: '4px 0 0' }}>Manage storewide, category, or product level promotional discounts</p>
        </div>
